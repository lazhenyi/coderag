...
impl Config {
    ...
    fn apply_json(config: &mut Config, json: JsonConfig) {
        if let Some(v) = json.qdrant_url { config.qdrant_url = Some(v); }
        if let Some(v) = json.qdrant_api_key { config.qdrant_api_key = Some(v); }
        if let Some(v) = json.openai_api_key { config.openai_api_key = Some(v); }
        if let Some(v) = json.embed_model { config.embed_model = Some(v); }
        if let Some(v) = json.embed_url { config.embed_url = Some(v); }
        if let Some(v) = json.embed_dimension { config.embed_dimension = Some(v); }
        if let Some(v) = json.repo_path { config.repo_path = Some(v); }
        if let Some(v) = json.branch { config.branch = Some(v); }
        if let Some(v) = json.state_file { config.state_file = Some(v); }
        if let Some(v) = json.collection_name { config.collection_name = Some(v); }
        if let Some(v) = json.batch_size { config.batch_size = Some(v); }
        if let Some(extra) = json.extra { config.extra = extra; }
    }

    /// Apply environment variables to Config
    fn load_env(config: &mut Config) {
        if let Ok(v) = std::env::var("QDRANT_URL") { config.qdrant_url = Some(v); }
        if let Ok(v) = std::env::var("QDRANT_API_KEY") { config.qdrant_api_key = Some(v); }
        if let Ok(v) = std::env::var("OPENAI_API_KEY") { config.openai_api_key = Some(v); }
        if let Ok(v) = std::env::var("CODERAG_EMBED_MODEL") { config.embed_model = Some(v); }
        if let Ok(v) = std::env::var("CODERAG_EMBED_URL") { config.embed_url = Some(v); }
        if let Ok(v) = std::env::var("CODERAG_EMBED_DIMENSION") { config.embed_dimension = v.parse().ok(); }
        if let Ok(v) = std::env::var("CODERAG_REPO") { config.repo_path = Some(v); }
        if let Ok(v) = std::env::var("CODERAG_BRANCH") { config.branch = Some(v); }
        if let Ok(v) = std::env::var("CODERAG_STATE_FILE") { config.state_file = Some(v); }
        if let Ok(v) = std::env::var("CODERAG_COLLECTION") { config.collection_name = Some(v); }
        if let Ok(v) = std::env::var("CODERAG_BATCH_SIZE") { config.batch_size = v.parse().ok(); }
    }
