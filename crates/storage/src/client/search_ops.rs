...
use super::types::{QdrantClient, ChunkPayload, SearchResult, SearchOptions, SearchFilter};
use anyhow::{Context, Result as AnyResult};
use serde::{Serialize, Deserialize};

impl QdrantClient {
    /// Upsert multiple points in batch
    pub async fn upsert_points_batch(&self, points: Vec<(String, Vec<f32>, ChunkPayload)>) -> AnyResult<()> {
        if points.is_empty() {
            return Ok(());
        }

        #[derive(Serialize)]
        struct UpsertRequest { points: Vec<Point> }

        #[derive(Serialize)]
        struct Point { id: String, vector: Vec<f32>, payload: ChunkPayload }

        let request = UpsertRequest {
            points: points.into_iter().map(|(id, vector, payload)| Point { id, vector, payload }).collect(),
        };

        let resp = self.add_auth(self.http.put(&self.collection_url("points")).json(&request)).send().await;
    ...
    pub async fn search(&self, vector: &[f32], options: SearchOptions) -> AnyResult<Vec<SearchResult>> {
        ...
        let resp = self.add_auth(self.http.post(&self.collection_url("points/search")).json(&request)).send().await;
        ...
        Ok(results.into_iter().filter_map(|r| {
            ...
            Some(SearchResult { id, score: r.score, payload: r.payload? })
        }).collect())
    }
    ...
    pub async fn delete_by_filter(&self, filter: SearchFilter) -> AnyResult<()> {
        #[derive(Serialize)]
        struct DeleteRequest { filter: SearchFilter }
        ...
        let resp = self.add_auth(
            self.http.post(format!("{}/points/delete", self.collection_url("")))
                .json(&request)
        ).send().await.context("Failed to delete by filter")?;
