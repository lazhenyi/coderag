...
use super::super::client::{SearchFilter, SearchOptions};
...
impl LocalStore {
    ...
    pub fn search(
    ...
    ) -> AnyResult<Vec<LocalSearchResult>> {
        let points = self.points.read().unwrap();
        let mut results: Vec<LocalSearchResult> = points
            .iter()
            .filter(|p| {
                ...
                if let Some(ref filter) = options.filter {
                    if let Some(ref lang) = filter.language {
                        if p.payload.language != *lang {
                            return false;
                        }
                    }
                    if let Some(ref file) = filter.file {
                        if !p.payload.file.contains(file) {
                            return false;
                        }
                    }
                    if let Some(ref kind) = filter.kind {
                        if p.payload.kind != *kind {
                            return false;
                        }
                    }
                    if let Some(ref repo) = filter.repo {
                        if p.payload.repo != *repo {
                            return false;
                        }
                    }
                }
        ...
        let limit = if options.limit > 0 {
            ...
            results.len().saturating_sub(offset)
        };
