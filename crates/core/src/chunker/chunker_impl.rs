...
impl Chunker {
    ...
    fn create_code_chunk(&self, symbol: &Symbol, repo: &str, branch: &str, commit: &str) -> Chunk {
        ...
        let content = format!(
            "{}{}{}{}{}",
            symbol.code, symbol.signature, symbol.file_path, symbol.module_path, symbol.language
        );
    ...
    fn create_doc_chunk(&self, symbol: &Symbol, repo: &str, branch: &str, commit: &str) -> Chunk {
        ...
        let content = format!(
            "{}{}{}{}{}",
            doc_content, symbol.signature, symbol.file_path, symbol.module_path, symbol.language
        );
        let content_hash = compute_hash(&content);
        Chunk {
            id,
