...
"#;
        let result = parser.parse_auto(rust_code.as_bytes(), Some(Path::new("test.rs"))).unwrap();
...
"#;
        let result = parser.parse_auto(python_code.as_bytes(), Some(Path::new("test.py"))).unwrap();
...
"#;
        let result = parser.parse_auto(js_code.as_bytes(), Some(Path::new("test.js"))).unwrap();
    ...
    fn test_language_detection() {
        ...
        assert_eq!(parser.detect_language_from_content(python_shebang), Some(Language::Python));

        let rust_shebang = b"#!/usr/bin/rust-run\nfn main() {}";
        assert_eq!(parser.detect_language_from_content(rust_shebang), Some(Language::Rust));
    }
}
