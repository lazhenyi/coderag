...
use super::types::{SerializableOid, CommitInfo, FileEntry, DiffCommitsResult};
...
impl GitRepo {
    pub fn open(path: impl AsRef<Path>) -> AnyResult<Self> {
        ...
        Ok(Self { repo, allowed_extensions: None })
    }
    ...
    pub fn find_commit(&self, oid: Oid) -> AnyResult<CommitInfo> {
        let commit = self.repo.find_commit(oid).context(format!("Failed to find commit {}", oid))?;
        let author = commit.author();
        Ok(CommitInfo {
            oid: SerializableOid::new(commit.id()),
            message: commit.summary().unwrap_or("").to_string(),
        ...
        for entry in tree.iter() {
            ...
            match entry.kind() {
                Some(git2::ObjectType::Blob) => {
                    ...
                    files.push(FileEntry { path: path.clone(), blob_id: SerializableOid::new(entry.id()), file_mode: entry.filemode() as i32 });
                }
    ...
    pub fn path(&self) -> &Path { self.repo.path() }
    pub fn workdir(&self) -> Option<&Path> { self.repo.workdir() }
    ...
    pub fn list_branches(&self) -> AnyResult<Vec<String>> {
        ...
        for branch_result in self.repo.branches(Some(git2::BranchType::Local))? {
            ...
            if let Some(name) = branch.name()? { branches.push(name.to_string()); }
        }
        Ok(branches)
    }
    ...
    pub fn diff_commits(&self, old_commit_oid: Oid, new_commit_oid: Oid) -> AnyResult<DiffCommitsResult> {
        let old_commit = self.repo.find_commit(old_commit_oid).context("Failed to find old commit")?;
        let new_commit = self.repo.find_commit(new_commit_oid).context("Failed to find new commit")?;
        ...
        let diff = self.repo.diff_tree_to_tree(Some(&old_tree), Some(&new_tree), Some(&mut diff_options))
            .context("Failed to compute diff")?;
        let mut files = Vec::new();
        diff.foreach(&mut |delta, _hunk| { files.push(unsafe { std::mem::transmute(delta) }); true }, None, Some(&mut |_delta, _hunk| true), None)?;
        let stats = diff.stats()?;
        Ok(DiffCommitsResult { old_commit: old_commit_oid, new_commit: new_commit_oid, files, insertions: stats.insertions() as usize, deletions: stats.deletions() as usize })
    }

    pub fn diff_trees(&self, old_tree: &Tree, new_tree: &Tree) -> AnyResult<git2::Diff<'_>> {
        let mut diff_options = git2::DiffOptions::new();
        diff_options.ignore_filemode(true);
        self.repo.diff_tree_to_tree(Some(old_tree), Some(new_tree), Some(&mut diff_options))
            .context("Failed to compute tree diff")
    }

    pub fn raw(&self) -> &Repository { &self.repo }
}
