//! XLS (BIFF) extraction using calamine

/// Extract text from .xls (BIFF format)
pub fn extract_xls(content: &[u8]) -> Option<String> {
    #[cfg(feature = "doc-p1")]
    {
        use calamine::{Data, Reader, Xls};
        use std::io::Cursor;

        let mut workbook: Xls<_> = match Xls::new(Cursor::new(content)) {
            Ok(w) => w,
            Err(e) => {
                tracing::warn!("Failed to open XLS: {}", e);
                return None;
            }
        };
