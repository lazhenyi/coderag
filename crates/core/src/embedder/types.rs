...
impl Default for EmbedderConfig {
    fn default() -> Self {
        Self {
            ...
            api_key: None, model: "text-embedding-ada-002".to_string(),
            dimension: 1536, batch_size: 100, timeout_secs: 60,
        }
    }
}

#[derive(Debug, Clone)]
pub enum EmbedderBackend { OpenAI, Local }
...
impl Embedding {
    pub fn new(vector: Vec<f32>, model: String) -> Self { Self { vector, model } }
    pub fn dimension(&self) -> usize { self.vector.len() }
    pub fn cosine_similarity(&self, other: &Embedding) -> f32 {
        if self.vector.len() != other.vector.len() { return 0.0; }
