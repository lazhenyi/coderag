...
use super::types::{EmbedderBackend, EmbedderConfig, Embedding};
    ...
    async fn embed_openai(&self, texts: &[String]) -> AnyResult<Vec<Embedding>> {
        if self.config.api_key.is_none() {
            ...
            return Ok(texts
                ...
                .map(|t| {
                    Embedding::new(
                        self.deterministic_vector(t),
                        format!("{}-fallback", self.config.model),
                    )
                })
                .collect());
        }
        ...
        let response = match req_builder.send().await {
            Ok(r) => r,
            Err(e) => {
                tracing::warn!("API request failed ({}), using fallback", e);
        ...
        let result: OpenAIResponse = match response.json().await {
            Ok(r) => r,
            Err(e) => {
                tracing::warn!("API parse failed ({}), using fallback", e);
                return self.fallback_all(texts);
            }
        };
        Ok(result
    ...
    async fn embed_local(&self, texts: &[String]) -> AnyResult<Vec<Embedding>> {
        ...
        for text in texts {
                ...
                .await
            {
                Ok(r) => r,
                Err(e) => {
                    tracing::warn!("Local request failed ({}), using fallback", e);
            ...
            let result: LocalResponse = match response.json().await {
                Ok(r) => r,
                Err(e) => {
                    tracing::warn!("Local parse failed ({}), using fallback", e);
                    embeddings.push(Embedding::new(
                        self.deterministic_vector(text),
                        format!("{}-fallback", self.config.model),
                    ));
